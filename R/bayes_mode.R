...
bayes_mode <- function(
  ...
  modes <- t(apply(mcmc, 1, mix_mode_estimates,
    dist = dist,
  ...
  if (dist_type == "discrete") {
    ...
    modes <- t(apply(mcmc, 1,
      FUN = mix_mode_estimates,
      range = range,
      dist = dist,
      dist_type = dist_type,
      tol_mixp = tol_mixp,
  ...
  # mixture density
  mix_density <- apply(mcmc, 1,
    FUN = dmix, x = mode_range,
