...
data {
  ...
  real<lower=0> d0;        // prior - distribution kappa
  real<lower=0> D0;        // prior - distribution kappa
}
...
parameters {
  ...
  positive_ordered[K] lambda;                            // poisson parameter
  vector<lower=0>[(e0>0) ? 0 : 1] alpha;                 // parameter mixing proportions
  simplex[max_y+1] kappa[K];                             // location parameters
  vector<lower=0>[(e0_kappa>0) ? 0 : 1] alpha_kappa;     // controls the distribution of location parameters
}
  ...
  for (n in 1:N) {
    ...
    for (k in 1:K) {
      ...
      for (j in 0:max_y) {
        ...
        if((y[n] - j) > -1){
          lp[j+1] = log_theta[k] + log_kappa[j+1] + poisson_lpmf(y[n] - j | lambda[k]); 
        } else {
          lp[j+1] = log(0);
        }
